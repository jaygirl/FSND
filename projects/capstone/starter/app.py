...
from models import setup_db, Movie, Actor, MovActor, db
...
def create_app(test_config=None):
    ...
    app = Flask(__name__)
    app.secret_key = "agency secret key"
    setup_db(app)
    ...
    auth0 = oauth.register(
    	'auth0',
    	client_id='K3l1DMKEtj4QFLTI0dJeStz5D2KA8T9p',
	    client_secret='3b2YXsGS4jNi5VPHhdrVK-Q2bMmuGNwhjdbDD5P5GCUaj89AS9DMj2vHX2iKNtMB',
	    api_base_url='https://fsnd-jay.us.auth0.com',
	    access_token_url='https://fsnd-jay.us.auth0.com/oauth/token',
	    authorize_url='https://fsnd-jay.us.auth0.com/authorize',
	    client_kwargs={
	    	'scope': 'openid profile email',
    	},
	)
  ...
  #   def callback_handling():
    ...
    def logout():
	    ...
	    session.clear()
	    # Redirect user to logout endpoint
	    params = {'returnTo': url_for('index', _external=True), 'client_id': 'K3l1DMKEtj4QFLTI0dJeStz5D2KA8T9p'}
	    return redirect(auth0.api_base_url + '/v2/logout?' + urlencode(params))
    ...
    def delete_actor(jwt, id):
        ...
        try:
            ...
            return jsonify({
                'success': True,
                'message':
                f'actor id {actor.id}, with name {actor.name}, was deleted',
            })
        except Exception:
            db.session.rollback()
            abort(500)
